...
module.exports = async function (databasePool, logger, discordId) {
  ...
  try {
    const sqlResult = (
      await databasePool.query(
        "SELECT ENCODE(refresh_token, 'hex') AS hex_refresh_token, salt FROM users WHERE user_id=$1;",
        [discordId]
      )
    ).rows[0];
    if (sqlResult.hex_refresh_token && sqlResult.salt) {
      ...
      const refreshToken = aesjs.utils.utf8.fromBytes(
        aesCtr.decrypt(aesjs.utils.hex.toBytes(sqlResult.hex_refresh_token))
      ); //The encrypted hex is converted to bytes, then decrypted, then converted to String
      ...
      if (oauthData.error == undefined && oauthData.message == undefined) {
        ...
        await databasePool.query(
          "UPDATE users SET token = DECODE($1, 'hex'), refresh_token = DECODE($2, 'hex') WHERE user_id = $3;",
          [encryptedToken, encryptedRefreshToken, discordId]
        );
        ...
        logger.warn(
          "Error when refreshing a token for the user " +
            discordId +
            " : Error = " +
            oauthData.error +
            "; Message = " +
    ...
    logger.error("Error when refreshing a token for the user " + discordId + " : " + err);
    return undefined;
  }
};
