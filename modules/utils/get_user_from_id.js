...
const regen_token = require("../discord_token_regen.js");
...
async function getUserDataFromAPI(token){
  ...
  try{
    result = await (await fetch('https://discord.com/api/users/@me', {
      headers: {
...
module.exports = async(database_pool, logger, userId)=>{
  logger.debug("We will now try to get data about user "+userId+" from Discord. Preparing SQL request to get token");

  let row;
  try{
    row = (await database_pool.query("SELECT ENCODE(token, 'hex') AS hex_token, salt FROM users WHERE user_id=$1;", [userId])).rows;
  }catch(err){
    logger.error("Error while geting user token from database : "+err);
    return undefined;
  }
  if(row.length!=1)throw("User not found");
  ...
  try{
    ...
    let apiResult = await getUserDataFromAPI(token);
    if(apiResult.id){
      ...
      const newToken = await regen_token(database_pool, logger, userId);
      if(newToken){
        //OK
        let apiResult2 = await getUserDataFromAPI(newToken);
        if(apiResult2.id){
          //OK
          return apiResult2;
        }else{
