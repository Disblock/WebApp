...
module.exports = async function (req, res, databasePool, logger) {
  ...
  if (
    req.session.securityToken === req.query.token &&
    req.session.securityToken != undefined &&
    Array.isArray(req.session.workingOnServer)
  ) {
    //We generate a security token to avoid some security problems
    req.session.securityToken = crypto.randomBytes(16).toString("hex");

    guildsDatabase
      .checkIfServerExist(databasePool, String(req.query.server_id), true)
      .then(async (result) => {
        ...
        for (
          let i = 0;
          i < req.session.workingOnServer.length;
          i++ //Iterate throught all user's admin guilds, and compare them to the ID of the selected guilds
        ) {
          if (req.session.workingOnServer[i] === String(req.query.server_id)) {
            //If one guild match this ID, the user is admin in this guild. If none match with, user isn't admin on it
        ...
        if (guild != undefined) {
          ...
          try {
            const currentPremium = await databasePool.query(
              "SELECT premium_id, server_id FROM premium WHERE premium_id=$1 AND user_id=$2 AND (end_date > NOW() OR end_date IS NULL);",
              [req.query.premium_id, req.session.discord_id]
            );
            if (currentPremium.rows.length == 1) {
              if (currentPremium.rows[0].server_id) {
                //There is an old guild for this premium slot
                const temp = await serverLogs.addEventToLogs(
                  databasePool,
                  currentPremium.rows[0].server_id,
                  req.session.discord_id,
                  serverLogs.eventType.removedPremium,
                  true,
                  false
                );
                if (!temp.correct) {
                ...
                await databasePool.query("UPDATE server_code SET active = FALSE WHERE server_id = $1;", [
                  currentPremium.rows[0].server_id,
                ]);
                logger.info(
                  req.session.discord_id +
                    " removed premium for guild " +
                    currentPremium.rows[0].server_id +
                    ". Guild code was disabled"
                );
              }
            } else {
            ...
            await serverLogs.addEventToLogs(
              databasePool,
              guild,
              req.session.discord_id,
              serverLogs.eventType.addedPremium,
              true,
              false
            );
          } catch (err) {
          ...
          databasePool
            .query(
              "UPDATE premium SET server_id = $1 WHERE premium_id = $2 AND user_id = $3 AND (end_date > NOW() OR end_date IS NULL);",
              [guild, req.query.premium_id, req.session.discord_id]
            )
            .then(async () => {
