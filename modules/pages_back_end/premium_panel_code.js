...
module.exports = async function(req, res, database_pool, logger){
  ...
  if(req.session.discord_id!=undefined && req.query.premium_code){
    ...
    database_pool.query("SELECT code, duration FROM premium_code WHERE code=$1 AND active = TRUE AND (expire_date > NOW() OR expire_date IS NULL);", [req.query.premium_code])
    .then(async(result)=>{
      if(result.rows.length==1){
        ...
        if(result.rows[0].duration){
          //Temporary
          duration = new Date(Date.now() + result.rows[0].duration*60000);//Ms to min
        }else{
          //Lifetime
          duration = null;
        }
        ...
        try{
          await database_pool.query("UPDATE premium_code SET active = FALSE WHERE code = $1;", [req.query.premium_code]);
          await database_pool.query("INSERT INTO premium (start_date, end_date, user_id, premium_code) VALUES (NOW(), $1, $2, $3);", [duration, req.session.discord_id, req.query.premium_code]);
