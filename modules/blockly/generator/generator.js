    ...
    */
    ...
    Blockly.JavaScript['block_embed_send'] = function(block) {
      ...
      if(value_embed!=='' && value_channel!==''){//GuildMember has a .user property, not the Channel objects
        const code = 'if('+value_channel+'.user) sentMessage = await '+value_channel+'.send({ embeds: [Discord.EmbedBuilder.from('+value_embed+').setFooter({iconURL: '+value_channel+'.guild.iconURL({dynamic:true}), text:"Sent from the server "+'+value_channel+'.guild.name})] });  \n\
