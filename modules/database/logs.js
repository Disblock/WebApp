  ...
  addEventToLogs: async function (databasePool, guildId, userId, eventType, premium, staff = false) {
    try {
      await databasePool.query(
        "INSERT INTO audit_log (server_id, user_id, action, action_date, staff_action) VALUES ($1, $2, $3, NOW(), $4);",
        [guildId, userId, eventType, staff]
      );
      ...
      await databasePool.query(
        "DELETE FROM audit_log WHERE audit_id IN (SELECT audit_id FROM audit_log WHERE server_id = $1 ORDER BY audit_id DESC OFFSET $2);",
        [guildId, maxLogs]
      );
