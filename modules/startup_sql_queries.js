"use strict";
module.exports = async function (databasePool) {
  ...
  await databasePool.query(
    "CREATE TABLE IF NOT EXISTS forms (\
      ...
      defined boolean NOT NULL DEFAULT false,\
      active boolean NOT NULL DEFAULT true,\
      CONSTRAINT fk_forms_server FOREIGN KEY (server_id) REFERENCES servers (server_id)\
    );"
  );

  await databasePool.query(
    "CREATE TABLE IF NOT EXISTS forms_args (\
      arg_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,\
      form_id bigint NOT NULL,\
      description varchar(128),\
      required boolean DEFAULT true,\
      type int NOT NULL,\
      CONSTRAINT fk_forms_args FOREIGN KEY (form_id) REFERENCES forms (form_id)\
    );"
  );
  ...
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_workspace_server ON server_workspace (server_id);");
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_server_code_server ON server_code (server_id);");
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_audit_log_server ON audit_log (server_id);");
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_error_message_server ON error_message (server_id);");
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_user_ban_user ON user_ban (user_id);");
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_server_ban_server ON server_ban (server_id);");
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_premium_server ON premium (server_id);");
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_premium_user ON premium (user_id);");
  await databasePool.query(
    "CREATE INDEX IF NOT EXISTS i_data_storage_server_and_storage ON data_storage (server_id, storage_name);"
  );
  await databasePool.query(
    "CREATE INDEX IF NOT EXISTS i_stored_data_storage_and_data ON stored_data (storage_id, data_key);"
  );
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_commands_server_and_name ON commands (server_id, name);");
  await databasePool.query("CREATE INDEX IF NOT EXISTS i_commands_args_command_id ON commands_args (command_id);");
  ...
  */
  await databasePool.query(
    "CREATE OR REPLACE FUNCTION f_update_data_storages_for_guild(f_serverid servers.server_id%TYPE, VARIADIC f_storagesNames VARCHAR[] DEFAULT ARRAY[]::VARCHAR[]) \
  RETURNS VOID \
  LANGUAGE PLPGSQL \
  AS $$ \
  DECLARE \
  ...
  */
  await databasePool.query(
    ...
    IF f_size > " +
      process.env.DATA_STORAGES_MAX_VARS +
      " THEN\
  ...
  */
  ...
  await databasePool.query(
    "INSERT INTO audit_log_actions VALUES (1, 'Updated Workspace'), (2, 'Rollbacked Workspace'), (3, 'Made this server Premium'), (4, 'Removed Premium') ON CONFLICT DO NOTHING;"
  );

  await databasePool.query("ALTER TABLE servers DROP COLUMN IF EXISTS name;");
  await databasePool.query("ALTER TABLE users DROP COLUMN IF EXISTS username;");
  await databasePool.query("ALTER TABLE users DROP COLUMN IF EXISTS avatar;");
};
